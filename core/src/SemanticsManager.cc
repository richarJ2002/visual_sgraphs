...
void SemanticsManager::Run()
{
    while (true)
    {
        ...
        {
            // Re-compute the transformation from ground to horizontal - maybe
            // global eq. changed
        ...
        // Re-associate semantic planes if they get close to each other :))
        // after optimization
    ...
    double                                          perpThreshDeg)
{
    ...
    for (size_t idx1 = 0; idx1 < facingWalls.size(); ++idx1)
        for (size_t idx2 = idx1 + 1; idx2 < facingWalls.size(); ++idx2)
        {
            ...
            // Check if wall pairs form a square, considering the
            ...
            {
                givenRoom =
                    std::make_pair(facingWalls[idx1], facingWalls[idx2]);
                return true;
            }
        }
...
void SemanticsManager::updateMapRoomCandidateToRoomGeo(KeyFrame *pKF)
{
    ...
    for (auto wall : allWalls)
    {
        ...
        double distance = Utils::calculateDistancePointToPlane(
    ...
    if (facingWalls.size() > 0)
        ...
        for (auto roomCandidate : allRooms)
        {
            ...
            Sophus::SE3f metaMarkerPose =
                roomCandidate->getMetaMarker()->getGlobalPose();
            ...
            for (auto facingWallsPair : facingWalls)
            {
                ...
                double distance1 = Utils::calculateDistancePointToPlane(
                    facingWallsPair.first->getGlobalEquation().coeffs(),
                    metaMarkerPose.translation().cast<double>());
                double distance2 = Utils::calculateDistancePointToPlane(
                    facingWallsPair.second->getGlobalEquation().coeffs(),
                    metaMarkerPose.translation().cast<double>());
                ...
                // Update closestPair2 if distance2 is smaller and it's not the
                // same facingWallsPair as closestPair1
            ...
            {
                if (closestPair1.first != nullptr &&
                    closestPair1.second != nullptr)
                {
                     ...
                     ORB_SLAM3::Room::roomVariant::ROOM)
            {
                ...
                if (closestPair2.first != nullptr &&
                    closestPair2.second != nullptr)
                {
...
void SemanticsManager::detectRoom_FreeSpaceCluster()
{
    ...
    for (const auto &cluster : clusters)
    {
        ...
        {
            int          closeCount = 0;
            // Check the distance between the cluster centroid and the wall
            // centroid
            const double centroidDistance =
                (wall->getCentroid().cast<double>() - clusterCentroid).norm();
            if (centroidDistance <
                sysParams->room_seg
                    .cluster_centroid_wall_centroid_distance_thresh)
            {
                // Check the minimum wall to all cluster points distance, and
                // only add if that minimum is below the threshold
                for (const auto &point : cluster)
                {
                    const double distance =
                        Utils::calculateDistancePointToPlane(
                            wall->getGlobalEquation().coeffs(),
                            point);
                    if (distance <
                ...
                // Require at least 30% of cluster points to be close to the
                // wall
            ...
            std::remove_if(
                ...
                [&](ORB_SLAM3::Plane *wall)
                {
                    ...
                    Eigen::Vector3d normal =
                        wall->getGlobalEquation().normal().cast<double>();
                    // Direction from cluster centroid to wall centroid
                    Eigen::Vector3d direction =
                        (wall->getCentroid().cast<double>() - clusterCentroid)
                            .normalized();
                    // Remove walls whose normal points away from the cluster
                    // centroid
        ...
        // Note: this is done by checking GetRoomWallPlaneById unordered-map in
        // Atlas
        closestWalls.erase(
            std::remove_if(closestWalls.begin(),
                           closestWalls.end(),
                           [&](ORB_SLAM3::Plane *wall) {
                               return mpAtlas->GetRoomWallPlaneById(
                                          wall->getId()) != nullptr;
                           }),
            closestWalls.end());
        ...
        ORB_SLAM3::Room *existedRoom =
            associateRooms(clusterCentroid, closestWalls);
        ...
        // If the walls close to the cluster and facing the cluster are not
        // already in the room, add them to the room
        for (const auto &wall : closestWalls)
        {
            // If the wall is not already in closestWalls, add it to the room
            if (std::find(roomWalls.begin(), roomWalls.end(), wall) ==
                roomWalls.end())
            {
        ...
        // Check room layout
        // [Hint] the free-space based room detection creates ONLY rooms and not
        // corridors, due to the lack of definition
        if (existedRoom->getRoomVariant() == Room::UNDEFINED)
        {
            ...
            if (roomWalls.size() >= 2)
            {
                ...
                // If there is at least one pair of facing walls, classify as
                // room
                if (!facingWalls.empty())
                {
                    existedRoom->setRoomVariant(ORB_SLAM3::Room::ROOM);
                    existedRoom->setName("Room#" +
                                         std::to_string(existedRoom->getId()));
                    std::cout << "[SemMgr] Structural Element #"
                              << existedRoom->getId()
                              << " got classified as a Room." << std::endl;
                }
            }
        }

        /**
         * 🚧 [vS-Graphs v.1.5] Archived room/corridor classification based on
         * wall count and perpendicularity
         */
        // if (facingWalls.size() == 1)
        // {
        //     existedRoom->setRoomVariant(ORB_SLAM3::Room::CORRIDOR);
        //     existedRoom->setName("Corridor#" +
        //     std::to_string(existedRoom->getId())); std::cout << "[SemMgr]
        //     Structural Element #" << existedRoom->getId() << " got classified
        //     as a Corridor." << std::endl;
        // }
        // else
        // {
        //     // Variables
        ...
        //     for (size_t idx1 = 0; idx1 < facingWalls.size(); ++idx1)
        //     {
        //         for (size_t idx2 = idx1 + 1; idx2 < facingWalls.size();
        //         ++idx2)
        //         {
        //             // Variables
        //             int perpCount = 0;
        ...
        //             // Check if wall pairs form a square, considering the
        //             perpendicularity threshold if
        //             (Utils::arePlanesPerpendicular(wall1P1, wall1P2))
        //                 perpCount++;
        ...
        //         existedRoom->setName("Room#" +
        //         std::to_string(existedRoom->getId())); std::cout << "[SemMgr]
        //         Structural Element #" << existedRoom->getId() << " got
