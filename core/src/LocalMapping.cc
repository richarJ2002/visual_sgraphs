...
void LocalMapping::KeyFrameCulling()
{
    ...
    unsigned long lastOptimizableKeyFrameId = mpCurrentKeyFrame->mnId;
    if (mbInertial)
    {
        int       temporalKeyFrameCount = 0;
        KeyFrame *p_oldestKeyFrame      = mpCurrentKeyFrame;
        while (temporalKeyFrameCount < Nd && p_oldestKeyFrame->mPrevKF)
        {
            p_oldestKeyFrame = p_oldestKeyFrame->mPrevKF;
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
    ...
    // Before this line we are not changing the map
    {
        ...
        const bool         imuWasInitialized = mpAtlas->isImuInitialized();
        unique_lock<mutex> lock(p_activeMap->mMutexMapUpdate);
        if ((fabs(mScale - 1.f) > 0.00001) || !mbMonocular)
        {
            Sophus::SE3f Twg(mRwg.cast<float>().transpose(),
                             Eigen::Vector3f::Zero());
            p_activeMap->ApplyScaledRotation(Twg, mScale, true);
            mpTracker->UpdateFrameIMU(mScale,
                                      vpKF[0]->GetImuBias(),
                                      mpCurrentKeyFrame);
        }
...
void LocalMapping::ScaleRefinement()
{
    ...
    if ((fabs(mScale - 1.f) > 0.002) || !mbMonocular)
    {
        Sophus::SE3f Tgw(mRwg.cast<float>().transpose(),
                         Eigen::Vector3f::Zero());
        p_activeMap->ApplyScaledRotation(Tgw, mScale, true);
        mpTracker->UpdateFrameIMU(mScale,
