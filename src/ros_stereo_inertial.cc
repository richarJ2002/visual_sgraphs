...
class ImageGrabber
{
  public:
    ...
    void GrabVoxbloxSkeletonGraph(
        const visualization_msgs::MarkerArray &msgSkeletonGraph);
...
int main(int argc, char **argv)
{
    ...
    nodeHandler.param<std::string>(node_name + "/sys_params_file",
                                   sys_params_file,
                                   "file_not_set");
    nodeHandler.param<std::string>(node_name + "/voc_file",
                                   voc_file,
                                   "file_not_set");
    nodeHandler.param<std::string>(node_name + "/settings_file",
                                   settings_file,
                                   "file_not_set");
    ...
    {
        ROS_ERROR("Please provide the YAML file containing system parameters "
                  "in the launch file!");
        ros::shutdown();
        return 1;
    }
    ...
    nodeHandler.param<bool>(node_name + "/enable_pangolin",
                            enable_pangolin,
                            true);
    ...
    nodeHandler.param<std::string>(node_name + "/frame_camera",
                                   frameCamera,
                                   "camera");
    nodeHandler.param<std::string>(node_name + "/frame_world",
                                   frameWorld,
                                   "world");
    nodeHandler.param<bool>(node_name + "/static_transform",
                            pubStaticTransform,
                            false);
    nodeHandler.param<std::string>(node_name + "/frame_building_component",
                                   frameBC,
                                   "build_comp");
    nodeHandler.param<std::string>(node_name + "/frame_structural_element",
                                   frameSE,
                                   "struc_elem");
    ...
    ros::Subscriber sub_imu =
        nodeHandler.subscribe("/imu", 1000, &ImuGrabber::GrabImu, &imugb);
    ros::Subscriber sub_img_left =
        nodeHandler.subscribe("/camera/left/image_raw",
                              500,
                              &ImageGrabber::GrabImageLeft,
                              &igb);
    ros::Subscriber sub_img_right =
        nodeHandler.subscribe("/camera/right/image_raw",
                              500,
    ...
    // nodeHandler.subscribe("/aruco_marker_publisher/markers", 1,
    //                                                   &ImageGrabber::GrabArUcoMarker,
    //                                                   &igb);

    // Subscriber for images obtained from the Semantic Segmentater
    ros::Subscriber sub_segmented_img =
        nodeHandler.subscribe("/camera/color/image_segment",
                              50,
                              &ImageGrabber::GrabSegmentation,
                              &igb);

    // Subscriber to get the mesh from voxblox
    ros::Subscriber voxblox_skeleton_mesh =
        nodeHandler.subscribe("/voxblox_skeletonizer/sparse_graph",
                              1,
                              &ImageGrabber::GrabVoxbloxSkeletonGraph,
                              &igb);
...
void ImageGrabber::SyncWithImu()
{
    ...
    while (1)
    {
            ...
            !mpImuGb->imuBuf.empty())
        {
            ...
            {
                Sophus::SE3f Tcw = pSLAM->TrackStereo(imLeft,
                                                      imRight,
                                                      tImLeft,
                                                      vImuMeas,
                                                      "",
    ...
    const segmenter_ros::SegmenterDataMsg &msgSegImage)
{
    ...
    try
    {
        cv_imgSeg = cv_bridge::toCvCopy(msgSegImage.segmentedImage,
    ...
    pcl_conversions::toPCL(msgSegImage.segmentedImageProbability,
                           *pclPc2SegPrb);
...
void ImageGrabber::GrabVoxbloxSkeletonGraph(
    const visualization_msgs::MarkerArray &msgSkeletonGraphs)
{
